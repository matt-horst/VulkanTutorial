...
class HelloTriangleApplication {
 ...
 private:
  ...
  std::array<vk::raii::Buffer, FRAMES_IN_FLIGHT> uniformBuffers;
  std::array<vk::raii::DeviceMemory, FRAMES_IN_FLIGHT> uniformBuffersMemory;
  std::array<void*, FRAMES_IN_FLIGHT> uniformBuffersMapped;
  ...
  void createUniformBuffers() {
    for (auto i = 0; i < FRAMES_IN_FLIGHT; i++) {
      ...
      uniformBuffersMemory[i] = std::move(bufferMemory);
  ...
  void copyBuffer(vk::raii::Buffer& srcBuffer, vk::raii::Buffer& dstBuffer, vk::DeviceSize size) {
    ...
    graphicsQueue.submit(vk::SubmitInfo{.commandBufferCount = 1, .pCommandBuffers = &*copyCommandBuffer});
