...
class HelloTriangleApplication {
 ...
 private:
    ...
    for (const auto& mesh : model.meshes) {
      for (const auto& primitive : mesh.primitives) {
        ...
        for (size_t i = 0; i < posAccessor.count; i++) {
          ...
          // Therefore, we need to flilp the Y coordinate
        ...
        for (size_t i = 0; i < indexCount; i++) {
          ...
          if (indexAccessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_SHORT) {
            index = *reinterpret_cast<const uint16_t*>(&indexData[indexStride * i]);
          } else if (indexAccessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_INT) {
            index = *reinterpret_cast<const uint32_t*>(&indexData[indexStride * i]);
          } else if (indexAccessor.componentType == TINYGLTF_COMPONENT_TYPE_UNSIGNED_BYTE) {
            index = *reinterpret_cast<const uint8_t*>(&indexData[indexStride * i]);
          }
  ...
  vk::raii::ImageView createImageView(vk::raii::Image& image, vk::Format format, vk::ImageAspectFlags aspectMask,
                                      uint32_t mipLevels) {
  ...
  void createTextureImage() {
    ...
    KTX_error_code result =
        ktxTexture_CreateFromNamedFile(TEXTURE_PATH.c_str(), KTX_TEXTURE_CREATE_LOAD_IMAGE_DATA_BIT, &kTexture);
    ...
    createImage(texWidth, texHeight, 1, vk::SampleCountFlagBits::e1, textureImageFormat, vk::ImageTiling::eOptimal,
  ...
  void generateMipMaps(vk::raii::Image& image, vk::Format imageFormat, int32_t width, int32_t height,
                       uint32_t mipLevels) {
    // vk::FormatProperties properties = physicalDevice.getFormatProperties(imageFormat);
    ...
    vk::raii::CommandBuffer commandBuffer = createSingleTimeCommands();
  ...
  void createImage(uint32_t width, uint32_t height, uint32_t mipLevels, vk::SampleCountFlagBits sampleCount,
  ...
  void createDescriptorSets() {
    for (auto& gameObject : gameObjects) {
      ...
      gameObject.descriptorSets = device.allocateDescriptorSets(descriptorSetAI);
  ...
  void createDescriptorPool() {
    std::array poolSizes{
        vk::DescriptorPoolSize{
            ...
            .descriptorCount = FRAMES_IN_FLIGHT * MAX_OBJECTS,
        },
        vk::DescriptorPoolSize{
    ...
    vk::DescriptorPoolCreateInfo poolCI{
        ...
        .maxSets = FRAMES_IN_FLIGHT * MAX_OBJECTS,
  ...
  void updateUniformBuffer(uint32_t frameIndex) {
    ...
    for (auto& gameObject : gameObjects) {
      ...
      gameObject.rotation.y += rotationSpeed * deltaTime;  // Slow rotation around Y axis
    ...
    vk::ImageMemoryBarrier barrier{
        ...
        .subresourceRange =
            vk::ImageSubresourceRange{
                .aspectMask = vk::ImageAspectFlagBits::eColor, .levelCount = mipLevels, .layerCount = 1},
  ...
  void createRenderPass() {
    ...
    vk::AttachmentDescription colorAttachmentResolve({}, swapchainSurfaceFormat.format, vk::SampleCountFlagBits::e1,
  ...
  void recordCommandBuffer(uint32_t imageIndex) {
    ...
    for (const auto& gameObject : gameObjects) {
      commandBuffers[frameIndex].bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipelineLayout, 0,
                                                    *gameObject.descriptorSets[frameIndex], nullptr);
